...
class TimestepEmbedding(pl.LightningModule):
    ...
    def __init__(self, time_steps, emb_dim) -> None:
        ...
        log_emb = -torch.arange(0, emb_dim, step=2) / emb_dim * torch.log(torch.tensor(10000.0))
...
class  SelfAttentionBlock(pl.LightningModule):
    ...
    def __init__(self, ch) -> None:
        ...
        self.query = nn.Conv2d(ch, ch, kernel_size=1, stride=1, padding=0)
        self.key = nn.Conv2d(ch, ch, kernel_size=1, stride=1, padding=0)
        self.value = nn.Conv2d(ch, ch, kernel_size=1, stride=1, padding=0)
        self.out = nn.Conv2d(ch, ch, kernel_size=1, stride=1, padding=0)

    def forward(self, x: torch.Tensor, *_args):
        B, C, H, W = x.shape
        h = self.normalize(x)
        q = self.query(h)
        k = self.key(h)
...
class UNet(pl.LightningModule):
    ...
    def __init__(
        ...
        for i, ch_mult in enumerate(ch_mults):
            ...
            if i != len(ch_mults) - 1:
                self.down_blocks.append(Downsample(in_ch=curr_ch))
                ch_buffer.append(curr_ch)
                curr_res //= 2
        ...
        for i, ch_mult in enumerate(reversed(ch_mults)):
            ...
            if i != len(ch_mults) - 1: 
                self.up_blocks.append(Upsample(in_ch=curr_ch))
                curr_res *= 2
...
if __name__ == '__main__':
    ...
    model = UNet(input_shape=(3, 32, 32),
                 time_steps=10,
                 channels=32,
                 num_res_blocks=2)
    print(model(inputs, times).shape)
